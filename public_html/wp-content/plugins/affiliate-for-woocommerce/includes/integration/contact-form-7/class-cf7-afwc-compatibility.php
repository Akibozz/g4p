...
if ( ! class_exists( 'CF7_AFWC_Compatibility' ) ) {
	...
	class CF7_AFWC_Compatibility {
		...
		public function skip_validation( $result = null, $tags = array() ) {
			...
			if ( empty( $_POST['afwc_email'] ) || ! $this->is_user_exists( wc_clean( wp_unslash( $_POST['afwc_email'] ) ) ) ) { // phpcs:ignore 
				return $result;
			}
...
JS;
		...
		/**
		 * Check whether the user is already exists by email.
		 *
		...
		public function is_user_exists( $email = '' ) {
			if ( empty( $email ) || ( email_exists( $email ) <= 0 ) ) {
				return $data;
			}

			$user = get_user_by( 'email', $email );

			return $user instanceof WP_User && is_callable( array( $user, 'exists' ) ) && $user->exists();
		}
		...
		public function posted_data( $data = array() ) {
			if ( empty( $data['afwc_email'] ) || ! $this->is_user_exists( $data['afwc_email'] ) ) {
				return $data;
			}
